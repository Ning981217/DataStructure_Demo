...
public:
    ...
    bool Insert(T_key key, T_val nVal);                         //插入
    bool Delete(T_key key);                                     //删除
    bool Update(T_key OrgKey, T_val nDstVal);                   //修改
    bool Find(T_key key, T_val &val);                           //查询
    void Clear();                                               //清空

    //void LMR(PNODE pNode);                                      //中序遍历  
    CQueue<T_val> LMR_LOOP();                                     //中序遍历
...
template<typename T_key, typename T_val>
inline bool CBST<T_key, T_val>::Insert(T_key key, T_val nVal)
{
    //空节点
    if (this->m_pRoot == nullptr)
    {
        this->m_pRoot = new NODE(key, nVal);
        if (this->m_pRoot != nullptr)
        {
            return true;
        }
    ...
    //插入数据
    *pNodeChild = new NODE(key, nVal, pNodeParent);
...
template<typename T_key, typename T_val>
inline CQueue<T_val> CBST<T_key, T_val>::LMR_LOOP()
{
    CStack<PNODE> stk;
    CQueue<T_val> qu;
...
/*template<typename T_key, typename T_val>
inline void CBST<T_key, T_val>::MLR(PNODE pNode)
{
    if (pNode != nullptr)
    {
        std::cout << pNode->m_val << "  ";
...
template<typename T_key, typename T_val>
inline void CBST<T_key, T_val>::MLR_LOOP()
{
    ...
    while (true)
    {
        ...
        {
            //输出自己
            std::cout << pTemp->m_val << "  ";
            //右子节点入栈
            if (pTemp->m_pRight != nullptr)
            {
                stk.push(pTemp->m_pRight);
            }
            //更新
            pTemp = pTemp->m_pLeft;
        }
...
template<typename T_key, typename T_val>
inline void CBST<T_key, T_val>::LRM(PNODE pNode)
{
    if (pNode != nullptr)
    {
        LRM(pNode->m_pLeft);
...
template<typename T_key, typename T_val>
inline void CBST<T_key, T_val>::LRM_LOOP()
{
    std::stack<PNODE> stk;

    //保存输出的节点
    PNODE pPreHandled = nullptr;

    auto pTemp = this->m_pRoot;
    while (true)
    {
        //左子节点入栈
        while (pTemp != nullptr)
        ...
        //访问栈顶数据
        if (stk.top()->m_pRight == nullptr || stk.top()->m_pRight == pPreHandled)
        {
            //没有右节点，或者已经输出了
            //打印自己
            std::cout << stk.top()->m_val << "  ";
            //更新输出的节点
            pPreHandled = stk.top();
            //节点出栈
            stk.pop();
        ...
        {
            //右节点处理
            pTemp = stk.top()->m_pRight;
        }
    }
}

template<typename T_key, typename T_val>
inline void CBST<T_key, T_val>::Level()
{
    PNODE pNode = this->m_pRoot;
    if (pNode == nullptr)
    {
    ...
    while (!qu.empty())
    {
        ...
        {
            qu.push(pTemp->m_pLeft);
        }
        if (pTemp->m_pRight != nullptr)
        {
            qu.push(pTemp->m_pRight);
        }
    }
}*/
